...
const VenueFavoriteButton: React.FC<VenueFavoriteButtonProps> = ({
  ...
  const { isFavorited, toggleFavorite, error, clearError, isAtMaxCapacity, remainingSlots } = useFavorites()
  ...
  const handleFavoriteToggle = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      console.log(`${isFavoritedStatus ? 'Removed' : 'Added'} "${venueName}" ${isFavoritedStatus ? 'from' : 'to'} favorites`)
    } catch (error) {
  ...
  const iconClasses = `
    ...
    ${isFavoritedStatus 
      ? variant === 'heart' ? 'fill-red-500 text-red-500' : 'fill-golden text-golden'
      ...
      <div className="relative">
        <div className="flex items-center gap-3">
          <button
            ...
            aria-label={isFavoritedStatus ? `Remove ${venueName} from favorites` : `Add ${venueName} to favorites`}
            title={
          ...
          <div className="flex flex-col">
            ...
            {!isFavoritedStatus && remainingSlots <= 2 && remainingSlots > 0 && (
        ...
        {showError && (error || (!isFavoritedStatus && isAtMaxCapacity)) && (
          <div className="absolute top-full left-0 mt-2 p-2 bg-red-50 border border-red-200 rounded-lg shadow-lg z-50 min-w-64">
            <p className="text-xs text-red-700">
              {error || 'Maximum of 5 favorites reached. Please remove a favorite first.'}
            </p>
          </div>
        )}
      </div>
